...
int detect_png(char *filepath, pic_data *out){
	...
	/*获取宽度，高度，位深，颜色类型*/  
	out->channels  = png_get_channels(png_ptr, info_ptr);   /*获取通道数*/  
	...
	{/*如果是RGB+alpha通道，或者RGB+其它字节*/   
		size *= (sizeof(unsigned char));  /* 每个像素点占4个字节内存 */  
	...
	{/* 如果是RGB通道 */
     /* 将其存为RGB+ALPHA通道 */
		...
		temp = (3 * out->width);/* 每行有4 * out->width个字节 */  
		for(i = 0; i < out->height; i++)  
		{  
			for(j = 0; j < temp; j += 3)  
			{/* 一个字节一个字节的赋值 */  
	...
	{/* 如果是gray+alpha通道 */
		...
		temp = (2 * out->width);/* 每行有4 * out->width个字节 */
		for(i = 0; i < out->height; i++)  
		{  
			for(j = 0; j < temp; j += 2)  
			{/* 一个字节一个字节的赋值 */  
	...
	{/* 如果是gray通道 */
		...
		temp = (1 * out->width);/* 每行有4 * out->width个字节 */
		for(i = 0; i < out->height; i++)  
		{  
			for(j = 0; j < temp; j += 1)  
			{/* 一个字节一个字节的赋值 */  
